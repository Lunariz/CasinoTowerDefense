...
public class SpriteGameObject : GameObject
{
    ...
    public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
    {
        if (!visible || sprite == null)
            return;
        sprite.Draw(spriteBatch, this.GlobalPosition + drawPosition, origin, color, 0, 255, scale.X, scale.Y, rotation);
    }
    ...
    public bool CollidesWith(SpriteGameObject obj, int coarsity = 5)
    {
        ...
        for (int x = 0; x < b.Width; x += coarsity)
        {
            for (int y = 0; y < b.Height; y += coarsity)
            {
    ...
    public bool CollidesWithPrecise(SpriteGameObject obj)
    {
        if (!this.Visible || !obj.Visible || !BoundingBox.Intersects(obj.BoundingBox))
            return false;
        Rectangle b = Collision.Intersection(BoundingBox, obj.BoundingBox);
